...
export class AuctionMonitorApp {
  ...
  public async start(): Promise<void> {
    ...
    try {
      const {token, userId }= await this.carOnSaleClient.authService('salesman@random.com','123test')
      const { total, items } =
        await this.carOnSaleClient.getRunningAuctions(token, userId);
        const numBids = items
    ...
    const averageAuctionProgress = auctionProgress / total
      ...
      this.logger.log(
        `Percentage of average auction progress: ${
          averageAuctionProgress * 100
        }`
      );
